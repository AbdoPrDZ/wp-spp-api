...
function create_prices_table() {
	global $wpdb;
	$table_name = $wpdb->prefix . 'spp_prices'; // Table name with WordPress prefix
  $offers_table = $wpdb->prefix . 'spp_offers'; // Assuming the offers table is named spp_offers
    ...
    `period` ENUM('hour', 'day', 'week', 'month', 'year') NOT NULL DEFAULT 'month',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		PRIMARY KEY (`id`),
...
function create_subscriptions_table() {
  ...
  $offers_table = $wpdb->prefix . 'spp_offers'; // Assuming the offers table is named spp_offers
  $prices_table = $wpdb->prefix . 'spp_prices'; // Assuming the prices table is named spp_prices
  $users_table = $wpdb->prefix . 'users'; // Assuming the users table is named wp_users
    ...
    FOREIGN KEY (`offer_id`) REFERENCES `$offers_table`(`id`) ON DELETE CASCADE
    -- FOREIGN KEY (`price_id`) REFERENCES `$prices_table`(`id`) ON DELETE CASCADE
	) $charset_collate;";
